	...
	public CheckInDTO save(CheckInDTO checkInDto) {
		...
		if (staffDto.getHttpStatus() == HttpStatus.OK) {
			...
			if (!staffDto.getShiftIds().contains(shiftId)) {
				...
				StaffEntity reqStaff = staffRepo.findOneByUsername((String)checkInDto.getListRequest().get(0));
				List<CheckInEntity> checkIns = checkInRepo.findAllByStaffId(reqStaff.getId());
